...
API_KEY = None  
ORG_ID = None    # Inicializar como None
...
def main():
    global API_KEY, ORG_ID, headers  # Usar global para modificar las variables definidas fuera de la función

    # Solicitar la API Key y el Org ID al usuario
    API_KEY = st.text_input("Enter your API Key:", type="password")
    ORG_ID = st.text_input("Enter your Org ID:")

    if not API_KEY or not ORG_ID:
        st.error("Please provide both API Key and Org ID.")
        st.stop()

    # Actualizar headers después de obtener API_KEY y ORG_ID
    headers["Authorization"] = f"Bearer {API_KEY}"
    headers["CodeGPT-Org-Id"] = ORG_ID

    st.title("API Documentation Analyzer")

    agentes = obtener_agentes(API_KEY,ORG_ID)
        ...
        with st.expander("Agent Prompt Analysis"):
            ...
            if analisis:
                ...
                if total_count > 0:
                    ...
                    if no_count > 0:
                        st.error(f"{no_count} out of {total_count} aspects need improvement. Modify the prompt and try again.")
                        return
                        ...
                        st.warning(f"{total_count} aspects were evaluated, but {total_count - yes_count} aspects are not clearly satisfactory. Consider improving the prompt.")
                        if st.button("Proceed anyway"):
                            st.success("Proceeding despite the warnings.")
                        else:
                            return
                else:
                    st.warning("No aspects were found in the prompt analysis.")
                    return
            else:
                st.error("Unable to analyze the prompt.")
                return
    else:
        st.error("Unable to retrieve the agent's prompt.")
        return
